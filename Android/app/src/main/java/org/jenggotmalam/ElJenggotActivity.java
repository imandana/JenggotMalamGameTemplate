...
public class ElJenggotActivity extends android.app.Activity implements SurfaceHolder.Callback
{
	...
	ElJenggotAudio mAudio = null;
	ElJenggotAccelerometer mAccelerometer = null;
	...
	{
		android.util.Log.v("ElJenggot", "Started onCreate function");
		...
		mView.setOnKeyListener(new android.view.View.OnKeyListener() { @Override public boolean onKey(android.view.View v, int keyCode, KeyEvent event)
		{
			...
			//android.util.Log.v("ElJenggot", "keyCode: " + keyCode + " - action: " + action + " - unicode: " + event.getUnicodeChar());
	...
	{
		if (mAudio == null) mAudio = new ElJenggotAudio(this);
		return mAudio;
	}
	...
	{
		if (mAccelerometer == null) mAccelerometer = new ElJenggotAccelerometer(this);
	...
	private class ElJenggotAccelerometer implements android.hardware.SensorEventListener
	{
		private SensorManager mSensorManager = null;
		private ElJenggotActivity mContext;
		private boolean mUseGyroscope = false;
		private boolean mUseAccelerometer = false;

		public ElJenggotAccelerometer(ElJenggotActivity context)
		{
			mContext = context;
		}
	...
	private class ElJenggotAudio
	{
		...
		public ElJenggotAudio(ElJenggotActivity context) { mContext = context; }
		...
		{
			//android.util.Log.d("Jenggot_AUDIO", "open apk file Offset: " + offset + " - size: " + size);
			try
			{
				Stream s = new Stream();
		...
		{
			//android.util.Log.d("Jenggot_AUDIO", "Control streamed audio - Mode: " + mode);
